...
import Data.List (span, concat, sort, findIndex)

data Value = Number Int | List [Value] deriving (Show)

norm (Number a) = [Number a]
norm (List a) = a
...
instance Ord Value where
    compare (Number a) (Number b)       = a `compare` b
    compare (List a) (List b)           = a `compare` b
    compare a b = norm a `compare` norm b
...
parseList (']':xs) = ([], xs)
parseList x =
    let (value, r:rest) = parse x
        (tail, rest') = parseList rest
    in  case r of   ']' -> ([value], rest)
                    ',' -> (value:tail, rest')
parseNumber x =
    let (value, tail) = span isDigit x
    in  (Number $ read value, tail)

parse ('[':xs)  =
    let (values, rest) = parseList xs
    in  (List values, rest)
parse x         = parseNumber x

parse' = fst . parse
pairs = map (\(a:b:c) -> (parse' a, parse' b)) $ batch 3 $ lines input

indexed = zip [1..] $ map (\(a, b) -> a `compare` b) pairs

answer1 = sum $ map fst $ filter (\(a, b) -> b == LT) indexed

--
two = List [List [Number 2]]
six = List [List [Number 6]]
additional = [two, six]
allPackets = concat (additional: map (\(a, b) -> [a, b]) pairs)
sorted = sort allPackets

answer2 =
    let (Just a) = findIndex (==two) sorted
        (Just b) = findIndex (==six) sorted
    in  (a + 1) * (b + 1)
